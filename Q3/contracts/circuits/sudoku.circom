...
template sudoku() {
    ...
    for(var i = 0; i < 9; i++) {
        for(var j = 0; j < 9; j++) {
            ...
            rpSolution[i][j] = RangeProof(32);
            rpSolution[i][j].range[0] <== 0;
            rpSolution[i][j].range[1] <== 9;
        }
    }
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            rpSolution[i][j].in <== solution[i][j];
            assert(rpPuzzle[i][j].out && rpSolution[i][j].out);
